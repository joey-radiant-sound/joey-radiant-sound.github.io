...
const schema = z.object({
  ...
  partnerFirstName: z.string().trim().max(100).optional().or(z.literal("")),
  partnerLastName: z.string().trim().max(100).optional().or(z.literal("")),
  email: z.string().trim().email("Enter a valid email").max(200),
  weddingDate: z.string().trim().min(1, "Required").max(40),
  guestCount: z.coerce.number().int().min(1, "Required").max(10000),
  venue: z.string().trim().min(1, "Required").max(200),
  venueAddress: z.string().trim().max(400).optional().or(z.literal("")),
  ...
  try {
    await sendContactEmail({
      ...
      fields: {
        ...
        lastName: d.lastName,
        partnerFirstName: d.partnerFirstName || "",
        partnerLastName: d.partnerLastName || "",
        email: d.email,
